...
def ridge_regression(formula, data=None, style="ridge", params={}):
    if data is None:
        raise ValueError( "The parameter 'data' must be assigned a non-nil reference to a Pandas DataFrame")
...
def bootstrap_ridge_regression( formula, data=None, samples=100, style="ridge", params={}):
    if data is None:
        raise ValueError( "The parameter 'data' must be assigned a non-nil reference to a Pandas DataFrame")
    ...
    for i in range( samples):
        sampling = data.sample(len(data), replace=True)
        results = ridge_regression( formula, data=sampling, style=style, params=params)
        coeffs.append( results[ "coefficients"])
        sigmas.append( results[ "sigma"])
        rs.append( results[ "r_squared"])
    ...
    result = ridge_regression( formula, data=data)
